...
async function initContracts() {
  const lotteryContract = await viem.deployContract("Lottery", [
    "LotteryToken",
    "LTO",
    TOKEN_RATIO,
  ...
  tokenAddress = await lotteryContract.read.paymentToken();
  ...
  const contractHasMinterRole = await tokenContract.read.hasRole([
    MINTER_ROLE as Address,
    contractAddress
  ]);
